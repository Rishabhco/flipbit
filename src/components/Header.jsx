      ...
      <header className="inset-x-0 top-0 z-50 bg-gray-800 text-gray-400 divide-y divide-gray-500/8">
        <nav className="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
          ...
          <div className="hidden lg:flex lg:flex-1 lg:justify-end">
            {
              !loggedIn ?
                <Link to="/login" className="text-sm font-semibold leading-6">Log in <span aria-hidden="true">&rarr;</span></Link>:
                    ...
                    <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                      {
                        userNavigation.map(item=>{
                          <Menu.Item>
                            {({ active }) => (
                              <Link to={item.href} className={classNames(active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700')}>{item.name}</Link>
                            )}
                          </Menu.Item>
                        })
                      }
          ...
          <Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-gray-800 text-gray-400 px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            ...
            <div className="mt-6 flow-root">
              <div className="-my-6 divide-y divide-gray-500/8">
                ...
                <div className="py-6">
                  {
                    !loggedIn ? <Link to="/login" className="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 hover:bg-gray-50 hover:text-gray-900">Log in</Link> 
                    : <div>
                      {userNavigation.map(item=>{
                        <Link to={item.href} className="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 hover:bg-gray-50 hover:text-gray-900">{item.name}</Link>
                      })}
                      </div>
                  }
                </div>
